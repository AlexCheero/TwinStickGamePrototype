...
class OverlapWFC : MonoBehaviour{
	...
	public void Generate() {
		...
		if (IsPrefabRef(training.gameObject)){
			GameObject o = CreatePrefab(training.gameObject, new Vector3(0,0f, 99999f), Quaternion.identity);
				...
				for (int x = 0; x < width; x++){
					if (rendering[x,y] == null){
						...
						if (v != 99 && v < training.tiles.Length){
							Vector3 pos = new Vector3(x*gridsize, 0f, y*gridsize);
							...
							if (fab != null){
								...
								tile.transform.localEulerAngles = new Vector3(0, 360 - (rot * 90), 0);
